...
public class SmartJournalApp {
    ...
    String hashedInputPassword = PasswordUtil.hashPassword(password);
        ...
        for (int i = 0; i < users.length; i++) {
            if (users[i].getEmail().equals(email) && users[i].getPassword().equals(hashedInputPassword)) {
                currentUser = users[i];
    ...
    SQLiteUserDAO.insertUser(newUser);
...
private static void editJournal(String date) {
        ...
        if (userJournals[i].getDate().equals(date)) {
            ...
            FileHandler.saveJournals(
                ...
                journalCount
                        ...
                        SQLiteJournalDAO.updateJournal(
                currentUser.getEmail(),
                date,
                newContent,
                newMood
    ...
    private static void handleTodayJournal(String today, boolean exists) {
        if (!exists) {
            ...
            journalCount++;
            FileHandler.saveJournals(currentUser.getEmail(), userJournals, journalCount);
            SQLiteJournalDAO.saveJournal(currentUser.getEmail(), newEntry);
